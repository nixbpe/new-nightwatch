...
- AppError constructor order is **accepted as status-first** by the user decision F001-ERR1 in [F-001](planning/F-001-workspace-onboarding.md): `AppError(status, code, message, details?)`, where `status` is the HTTP status, `code` is machine-readable, `message` is display-safe, and `details` is optional and display-safe. The current [implementation](../packages/shared/src/errors.ts) is still code-first; this is an implementation-alignment gap, not an unresolved contract choice. When implementation is assigned, migrate the constructor, all callsites and affected tests together without a compatibility overload. This documentation change does not perform or authorize that migration.
