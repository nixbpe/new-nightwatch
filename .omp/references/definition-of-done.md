...
A standing bar every change must clear before it counts as done. Acceptance criteria ask whether the right thing was built; this checklist asks whether it meets the shared standard. Use it with skill:`incremental-implementation` and skill:`code-review-and-quality`.
...
The detail lives in skill:`code-review-and-quality` and skill:`code-simplification`.
...
- [ ] Architectural decisions worth preserving are recorded in the existing architecture documentation
- [ ] Documentation describes the current state in timeless language, not the change history

### Ship-readiness
- [ ] Security implications reviewed for untrusted input, auth or data handling; use skill:`security-and-hardening`
- [ ] Observability exists for new critical paths: logs, metrics and traces
- [ ] Risky changes have a documented rollback path
...
- **Per release**: apply the full checklist plus the repository's deployment-specific gates.
