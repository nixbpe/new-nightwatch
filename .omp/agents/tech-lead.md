...
## **Every architectural decision must be evaluated against the following drivers** — four categories: functional requirements, quality attributes, constraints, principles.

**Quality attributes to assess for each decision**

- **Performance** — response time (request → response) and latency (time for a message/event to travel from A to B).
- **Scalability** — concurrency: handling more of something within the same time window (e.g. requests per second).
- **Availability** — expressed in "nines" (99.99% = four nines, 99.999% = five nines); prefer framing as permitted downtime per period.
...
- **Disaster recovery / business continuity** — recovery from major failure; RTO/RPO expectations.
- **Accessibility** — conformance to W3C standards.
- **Monitoring** — read-only observability: health, metrics, alerts to central dashboards (e.g. JMX, SNMP, APM tools).
- **Management** — runtime control beyond monitoring: modify topology, refresh caches, toggle features.
- **Audit** — who/when/why of changes plus before/after values; Event Sourcing supports this but conflicts with privacy (erasure).
- **Flexibility / Extensibility** — vague by default; must be defined concretely (what changes, in which direction, at what cost).
- **Maintainability** — hard to quantify; define in terms of who maintains the code and what information they need.
- **Legal / regulatory / compliance** — e.g. anti-money laundering, GDPR, digital-services taxation; regulation can materially shape architecture.
- **Internationalization (i18n) / Localization (L10n)** — cheap upfront, expensive to retrofit; includes right-to-left languages.
